...
class LearnDash_Zarinpal_Settings
{
    ...
    public function admin_notices()
    {
        echo '<div class="error"><p>' . __(
                'LearnDash plugin is required to activate LearnDash Zarinpal add-on plugin. Please activate it first.',
                'learndash-zarinpal'
            ) . '</p></div>';
    }
    ...
    {
        if ( ! is_admin(
            ) || ! isset($_GET['page']) || (isset($_GET['page']) && 'learndash-zarinpal-settings' != $_GET['page'])
        ) {
            return;
        }
        ...
        wp_enqueue_script(
            'sfwd-module-script',
            LEARNDASH_LMS_PLUGIN_URL . 'assets/js/sfwd_module.js',
            array('jquery'),
            LEARNDASH_VERSION,
            true
        );
        ...
        // Load our admin JS
        wp_enqueue_script(
            'learndash-zarinpal-admin-script',
            LEARNDASH_ZARINPAL_PLUGIN_URL . 'assets/js/learndash-zarinpal-admin-script.js',
            array('jquery'),
    ...
    {
        register_setting(
            'learndash_zarinpal_settings_group',
            'learndash_zarinpal_settings',
    ...
    public function admin_menu()
    {
        add_submenu_page(
            ...
            __('Zarinpal Settings', 'learndash-zarinpal'),
            __('Zarinpal Settings', 'learndash-zarinpal'),
            'manage_options',
            'admin.php?page=learndash-zarinpal-settings',
            array($this, 'zarinpal_settings_page')
        );
    ...
    public function zarinpal_settings_page()
    {
        ...
        ?>
        <div class="wrap">
            <h2 class="learndash-zarinpal-settings-header"><?php
                _e('zarinpal Settings', 'learndash-zarinpal'); ?></h2>
            <form method="post" action="options.php">
                <div class="sfwd_options_wrapper sfwd_settings_left">
                    <div id="advanced-sortables" class="meta-box-sortables">
                        <div id="sfwd-courses_metabox" class="postbox learndash-zarinpal-settings-postbox">
                            ...
                            _e('Click to toggle', 'learndash-zarinpal'); ?>"><br></div>
                            <h3 class="hndle"><span><?php
                                    _e('Zarinpal Settings', 'learndash-zarinpal'); ?></span></h3>

                            <div class="inside">
                                <div class="sfwd sfwd_options sfwd-courses_settings">
                                    <?php
                                    settings_fields('learndash_zarinpal_settings_group'); ?>
                                    <?php
                                    foreach ($options as $key => $option) : ?>
                                        <?php
                                        $option['id'] = $key; ?>
                                        <div class="sfwd_input " id="sfwd-zarinpal_<?php
                                        echo $key; ?>">
											<span class="sfwd_option_label"
                                                  style="text-align:right;vertical-align:top;">
												<a class="sfwd_help_text_link" style="cursor:pointer;"
                                                   title="Click for Help!"
                                                   onclick="toggleVisibility('sfwd-zarinpal_<?php
                                                   echo $key; ?>_tip');">
													<img src="<?php
                                                    echo LEARNDASH_ZARINPAL_PLUGIN_URL . 'assets/images/question.png' ?>">
													<label class="sfwd_label textinput"><?php
                                            ...
                                            <span class="sfwd_option_input">
												<div class="sfwd_option_div">
													...
													<?php
                                                    $callback = $option['type'] . '_callback'; ?>

                                                    <?php
                                                    $this->$callback($option); ?>
											<span style="font-weight: bold;font-size: 1.2em"><?php echo $option['left'] ?? '' ?> </span>

												</div>
												<div class="sfwd_help_text_div" style="display:none"
                                                     id="sfwd-zarinpal_<?php
                                                     echo $key; ?>_tip"><label class="sfwd_help_text"><?php
                                                        echo $option['desc']; ?></label></div>
                                            </span>
                                        </div>
                ...
                <p class="submit" style="clear: both;">
                    <?php
                    submit_button(__('Update Options »', 'learndash-zarinpal'), 'primary', 'submit', false); ?>
                </p>
            </form>
        </div>
        <?php
    ...
    public function get_zarinpal_settings()
    {
        $settings = array(
            'enabled'    => array(
                ...
                'type'    => 'checkbox',
                'left' => 'بله',
                'desc'    => 'برای فعال‌سازی درگاه زرین پال، این گزینه را علامت بزنید.',
                'value'   => $this->options['enabled'] ?? false,
                'default' => false,
                'options' => array(
                    'on'  => true,
                    'off' => false,
    ...
    {
        $html = '<input type="checkbox" name="learndash_zarinpal_settings[' . $args['id'] . ']" value="1" ' . checked(
                $this->options[$args['id']] ?? false,
                1,
                false
            ) . '>';
